...
export default function Home() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages((prev) => [
        { question: q, answer: res.ok ? data.answer : (data.error ?? "오류가 발생했습니다.") },
        ...prev,
      ]);
    } catch {
      setMessages((prev) => [
        { question: q, answer: "서버에 연결할 수 없습니다. Python 서버가 실행 중인지 확인하세요." },
        ...prev,
      ]);
    } finally {
      setIsLoading(false);
          ...
          <div className="w-full max-w-2xl flex flex-col gap-4">
            {messages.map((msg, i) => (
              <div key={i} className="flex flex-col gap-2">
                ...
                <div className="self-start max-w-[80%] px-4 py-3 rounded-2xl bg-gray-100 text-gray-900 text-sm leading-relaxed">
                  {msg.answer}
                </div>
              </div>
            ))}
