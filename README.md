...
**품질 지표** — 단위 테스트 51개 통과 (0.1초) · 에이전트 회귀 평가 12/13 통과 · 의존성 79개 전체 고정
...
- **테스트**: pytest 단위 테스트 51개 + 골든 데이터셋 회귀 평가 13개 (LLM-as-judge)
...
```
골든 데이터셋 13개에 대해 **도구 호출이 기대대로인지 + 키워드가 답변에 있는지**를 확인하고,
...
- **테스트 두 층** — 순수 함수 단위 테스트 51개(0.1초, 비용 0)와 에이전트 회귀 평가 13개(LLM-as-judge)를 분리 운영. 테스트를 붙이기 위해 순수 로직을 `kobis_format.py`/`sources.py`로 분리해 `agent.py`의 무거운 초기화 없이 검증합니다
